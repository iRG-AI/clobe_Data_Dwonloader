...
function sendTelegram(cfg, message) {
  ...
  const reqPath = `/bot${cfg.telegram.botToken}/sendMessage?chat_id=${cfg.telegram.chatId}&text=${text}&parse_mode=HTML`;
  https.get({ hostname: 'api.telegram.org', path: reqPath }, () => {}).on('error', () => {});
}
...
const server = http.createServer((req, res) => {
  ...
  const cors = { ...CORS };
  if (!checkRate(ip)) { res.writeHead(429, cors); res.end(JSON.stringify({ error: '요청 과다' })); return; }
  ...
  if (url.pathname === '/run' && req.method === 'POST') {
    if (runningProc) { res.writeHead(409, cors); res.end(JSON.stringify({ error: '이미 실행 중입니다.' })); return; }
    let body = '';
    ...
    req.on('end', () => {
      ...
      function writeLog(line) { const t = new Date().toLocaleTimeString('ko-KR'); logStream.write(`[${t}] ${line}
`); }
      writeLog(`▶ clobe 업데이트 시작 (연도: ${year}년)`);
...
').filter(Boolean).forEach(l => writeLog(`[ERR] ${l}`)));
      runningProc.on('close', code => {
        const ok = code === 0;
        writeLog(ok ? '✅ 완료!' : `❌ 오류 (종료코드: ${code})`);
        logStream.end(); runningProc = null;
        sendTelegram(cfg, ok ? `✅ clobe 재무 업데이트 완료 (${year}년)` : `❌ clobe 재무 업데이트 실패 (${year}년) — 코드: ${code}`);
      });
      runningProc.on('error', err => { writeLog(`[SYS] 프로세스 오류: ${err.message}`); logStream.end(); runningProc = null; });
      res.writeHead(200, cors); res.end(JSON.stringify({ ok: true, logFile: path.basename(currentLogFile) }));
    });
    return;
  }
  ...
  if (url.pathname === '/logs/stream' && req.method === 'GET') {
    res.writeHead(200, { 'Content-Type': 'text/event-stream', 'Cache-Control': 'no-cache', 'Access-Control-Allow-Origin': 'http://localhost:3001', 'Connection': 'keep-alive' });
    if (!currentLogFile || !fs.existsSync(currentLogFile)) { res.write('data: {"type":"info","msg":"로그 없음"}

'); res.end(); return; }
    const content = fs.readFileSync(currentLogFile, 'utf8');
    content.split('
').filter(Boolean).forEach(line => res.write(`data: ${JSON.stringify({ type: 'log', msg: line })}
...
`));
      ...
      if (!runningProc) { res.write(`data: ${JSON.stringify({ type: 'done' })}
...
`); clearInterval(interval); res.end(); }
  ...
  if (url.pathname === '/logs' && req.method === 'GET') {
    const files = fs.existsSync(LOG_DIR) ? fs.readdirSync(LOG_DIR).filter(f => f.endsWith('.log')).sort().reverse().slice(0, 20) : [];
    res.writeHead(200, cors); res.end(JSON.stringify(files)); return;
  }
  ...
  if (url.pathname.startsWith('/logs/') && req.method === 'GET') {
    ...
    if (!fs.existsSync(fpath) || !fname.endsWith('.log')) { res.writeHead(404, cors); res.end(JSON.stringify({ error: 'Not Found' })); return; }
    res.writeHead(200, { ...cors, 'Content-Type': 'text/plain; charset=utf-8' });
    res.end(fs.readFileSync(fpath, 'utf8')); return;
  }
  ...
  if (url.pathname === '/config' && req.method === 'GET') {
    ...
    res.writeHead(200, cors); res.end(JSON.stringify(safe)); return;
  }
...
server.listen(PORT, '127.0.0.1', () => {
  ...
  console.log('  종료: Ctrl+C'); console.log('========================================'); console.log('');
});
...
if (process.argv.includes('--scheduler')) {
  ...
  const slog = msg => { const t = new Date().toLocaleTimeString('ko-KR'); const line = `[${t}] ${msg}`; console.log(line); logStream.write(line + '
'); };
...
').filter(Boolean).forEach(l => slog(`[ERR] ${l}`)));
  proc.on('close', code => {
    const ok = code === 0; slog(ok ? '✅ 완료!' : `❌ 오류 (코드: ${code})`); logStream.end();
    sendTelegram(cfg, ok ? `✅ clobe 재무 업데이트 완료 (${year}년) — ${new Date().toLocaleString('ko-KR')}` : `❌ clobe 재무 업데이트 실패 (${year}년) — 코드: ${code}`);
    process.exit(code);
  });
